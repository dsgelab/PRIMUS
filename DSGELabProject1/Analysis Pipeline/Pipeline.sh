...
outcome_register=''     # possible values: 'Prescription', 'Purchase'
...
if [ "$event_register" == "Diag" ]; then
    ...
    python3 $base_dir/AnalysisPipeline/ExtractEvents.py --id_list $list_of_doctors_spouses_children --inpath $base_dir/$filtered_THL_diagnosis --outdir $out_dir --event_code $event_code
elif [ "$event_register" == "Purch" ]; then
    echo "ATC code: $event_code"
    python3 $base_dir/AnalysisPipeline/ExtractEvents.py --id_list $list_of_doctors_spouses_children --inpath $base_dir/$filtered_kela_purchases --outdir $out_dir --event_code $event_code
else
...
# Step 3: Extract required outcome
echo "Extracting required outcome (Prescription)"
echo "ATC code: $outcome_code"
step_start_time=$SECONDS
python3 $base_dir/AnalysisPipeline/ExtractOutcomes.py --id_list $list_of_doctors --long_file_path $base_dir/$longitudinal_file --outdir $out_dir --outcome_register $outcome_register --outcome_code $outcome_code
step_end_time=$SECONDS
echo "Step completed in $(($step_end_time - $step_start_time)) seconds"
...
step_start_time=$SECONDS
Rscript --vanilla DiD_analysis.R $list_of_doctors $map_relatives $out_dir/ID_cases.csv $out_dir/ID_controls.csv $out_dir/Events.csv $out_dir/Outcomes.csv $out_dir
step_end_time=$SECONDS
echo "Step completed in $(($step_end_time - $step_start_time)) seconds"
