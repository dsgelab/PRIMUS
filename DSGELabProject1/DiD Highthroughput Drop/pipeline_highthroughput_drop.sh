...
# Event codes
event_codes_file="$base_dir/DiD_Highthroughput/Version1_drop/list_of_codes_20250919.csv"

# Output directories
today=$(date '+%Y%m%d')
experiment_dir="$base_dir/DiD_Experiments/Version1_Highthroughput_drop"
...
if [[ -f "$processed_events_dir/processed_events.parquet" ]]; then
    ...
    python3 "$base_dir/DiD_Highthroughput/Version1_drop/ProcessEvents.py" \
...
if [[ -f "$processed_outcomes_dir/processed_outcomes.parquet" ]]; then
    ...
    python3 "$base_dir/DiD_Highthroughput/Version1_drop/ProcessOutcomes.py" \
        --id_list "$list_of_doctors" \
...
echo "EVENT_CODE,BASELINE,DROP,TTR,N_CASES,N_CONTROLS" > "$results_file"
...
for ((i=0; i<total_events; i++)); do
    ...
    Rscript --vanilla "$base_dir/DiD_Highthroughput/Version1_drop/DropAnalysis.R" \
        "$list_of_doctors" \
