# Info
# This script takes all the pre-process hidiagnosis files (hilmo/avohilmo) and joins them to the respective doctor, and then push to one
# unique CSV file
...
# Paths
InDir = Path("/media/volume/Projects/DSGELabProject1/ProcessedData/")
hilmo_file = InDir / "processed_hilmo_20250218.csv"
avohilmo_file = InDir / "processed_avohilmo_20250218.csv"
...
for dataset in datasets:
    ...
    for chunk_index, chunk in enumerate(
        ...
        processed_data["VISIT_DATE"] = pd.to_datetime(
            processed_data[dataset.visit_date_column], format="%d.%m.%Y"
        )
