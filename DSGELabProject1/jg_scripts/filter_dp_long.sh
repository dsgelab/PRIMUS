...
TODAY=$(date +%Y-%m-%d)
OUTPUT_FILE="/media/volume/Projects/DSGELabProject1/filtered_dp_longitudinal_${TODAY}.csv.gz"
LOG_FILE="/media/volume/Projects/jg/Logs/filter_dp_longitudinal_log_${TODAY}.log"
...
zcat "$INPUT_FILE" | awk -F',' -v date_col="$DATE_COL" -v reg_col="$REGISTER_COL" '
    ...
    $date_col != "" && $date_col >= "2012-01-01" && ($reg_col == "Prescription" || $reg_col == "Purchase")
