...
if [[ -f "$processed_events_dir/processed_events.parquet" ]]; then
    ...
    python3 "$base_dir/DiD_Highthroughput/Version3/Step1_ProcessEvents.py" \
...
if [[ -f "$processed_outcomes_dir/processed_outcomes.parquet" ]]; then
    ...
    python3 "$base_dir/DiD_Highthroughput/Version3/Step2_ProcessOutcomes.py" \
        --id_list "$list_of_doctors" \
...
for ((i=0; i<total_pairs; i++)); do
    ...
    Rscript --vanilla "$base_dir/DiD_Highthroughput/Version3/DiD_analysis_highthroughput.R" \
