...
output_file <- sprintf('/media/volume/Projects/DSGELabProject1/DiD_Experiments/Version2_Highthroughput/Results/Manhattan_%s_%s.png', TYPE, DATE)
...
p <- ggplot(manhattan_data, aes(x = event_code, y = log10_p, color = event_group)) +
    ...
    ggrepel::geom_text_repel(
        data = subset(manhattan_data, !is.na(bonferroni_label)),
    ...
    labs(
        ...
        subtitle = paste0("Dashed: p < 0.05  |  Dotted: Bonferroni (p < ", format(bonferroni_threshold, scientific = TRUE, digits = 3), ")  |  Matrix size: ", length(event_codes), "×", length(outcome_codes)
        ),
        x = "Event Code",
