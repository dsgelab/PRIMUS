...
# Step 2: sort with more RAM, spill to TMPDIR if needed
sort --parallel=$(nproc) -S75% --temporary-directory="$TMPDIR" -t, -k1,1 -k2,2 step1.csv > step2.csv

# Step 3: keep first ATC event per PATIENT_ID
awk -F, '
BEGIN {OFS=","; print "PATIENT_ID,PURCHASE_DATE,ATC_CODE"}
NR > 1 && !seen[$1]++ {
    print
