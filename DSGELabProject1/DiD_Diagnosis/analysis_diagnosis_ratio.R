...
agg_dynamic_log <- aggte(att_gt_res, type = "dynamic", na.rm = TRUE)
results_log <- data.frame(
    time = agg_dynamic_log$egt,
    att_log = agg_dynamic_log$att.egt,
    se_log = agg_dynamic_log$se.egt
) %>%
    mutate(
        pct_change = (exp(att_log) - 1) * 100,
        pct_lower = (exp(att_log - 1.96 * se_log) - 1) * 100,
        pct_upper = (exp(att_log + 1.96 * se_log) - 1) * 100
    )

# For diagnosis results will only consider ATT and SE at event
effect_at_event <- results_log$pct_change[results_log$time == 0]
icd95_lower <- results_log$pct_lower[results_log$time == 0]
icd95_upper <- results_log$pct_upper[results_log$time == 0]
...
summary_row <- data.frame(
        ...
        pct_change = effect_at_event,
        pct_lower = icd95_lower,
        pct_upper = icd95_upper,
        n_cases = n_cases,
