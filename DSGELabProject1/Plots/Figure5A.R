...
# Global Variables
DATE = "20251218"
dataset_file <- '/media/volume/Projects/DSGELabProject1/DiD_Experiments/Version4_Highthroughput/Results/Results_ATC_20251117.csv'
...
dataset$SIG_TYPE <- factor(dataset$SIG_TYPE, levels = c("FDR Significant", "Not Significant"))
...
code_labels <- tibble(
    OUTCOME_CODE = c("C10AA07"),
    LABEL = c("rosuvastatin")
)
top_extreme <- dataset %>% inner_join(code_labels, by = "OUTCOME_CODE")
...
p_left <- ggplot(dataset, aes(x = x_jittered, y = EFFECT_DIFF, color = CHAPTER_NAME)) +
  ...
  geom_text_repel(data = top_extreme, aes(label = paste0(OUTCOME_CODE, ": ", LABEL)), 
                    size = 3, 
                    show.legend = FALSE,
                    max.overlaps = Inf,
  ...
  guides(shape = guide_legend(override.aes = list(size = c(POINT_SIZE_FDR, POINT_SIZE_NOT_SIG), alpha = c(ALPHA_FDR, ALPHA_NOT_SIG)))) +
...
# Save
ggsave(paste0(OutDir, "Figure5A_V1_", DATE, ".png"), plot = p_combined, width = 19, height = 12, dpi = 300, device = "png")

# ============================================================================
# Version 2: show oth FDR and nominally significant points with labels
# ============================================================================
...
# Select all FDR and p-value significant points for labeling
top_extreme <- dataset %>% 
    filter(SIGNIFICANT_FDR | PVAL < 0.05) %>%
    mutate(LABEL = OUTCOME_CODE)
...
# Save
ggsave(paste0(OutDir, "Figure5A_V2_", DATE, ".png"), plot = p_combined, width = 19, height = 12, dpi = 300, device = "png")
