...
# Load data
DATE = "20251218"
data <- read.csv(paste0("/media/volume/Projects/DSGELabProject1/DiD_Experiments/Version3_Highthroughput_drop/Results/DropAnalysisResults_20251118.csv"))
OutDir <- paste0("/media/volume/Projects/DSGELabProject1/Plots/Figure3_", DATE, "/")
if (!dir.exists(OutDir)) {dir.create(OutDir, recursive = TRUE)}

# ============================================================================
# Version 1: only show FDR significant points with labels
# ============================================================================

# Filter only codes with at least 300 cases available
data <- data[data$N_CASES >= 300, ]
...
data_diag$SIG_TYPE <- factor(data_diag$SIG_TYPE, levels = c("FDR Significant", "Not Significant"))
...
top_extreme <- data_diag %>% filter(SIGNIFICANT_FDR) %>% inner_join(code_labels, by = "EVENT_CODE")
...
p_left <- ggplot(data_diag, aes(x = x_jittered, y = ATT_DROP, color = CHAPTER_NAME)) +
    ...
    geom_text_repel(data = top_extreme, aes(label = paste0(gsub("Diag_", "", EVENT_CODE), ": ", LABEL)),
                        size = 3, 
                        show.legend = FALSE,
                        max.overlaps = Inf,
    ...
    scale_fill_manual(
        values = c("FDR Significant" = "black", "Not Significant" = "gray70"),
        guide = "none"
    ) +
    scale_shape_manual(
        name = "Significance",
        values = c("FDR Significant" = 17, "Not Significant" = 16)
    ) +
    scale_size_manual(
        name = "Significance",
        values = c("FDR Significant" = POINT_SIZE_FDR, "Not Significant" = POINT_SIZE_NOT_SIG)
    ) +
    scale_alpha_manual(
        name = "Significance",
        values = c("FDR Significant" = ALPHA_FDR, "Not Significant" = ALPHA_NOT_SIG)
    ) +
    labs(
        title = "Individual Diagnoses",
        x = "ICD-10 Diagnosis Chapter",
        y = "Absolute Change in Number of Prescriptions"
    ) +
    theme_minimal() +
    theme(
    ...
    guides(shape = guide_legend(override.aes = list(size = c(POINT_SIZE_FDR, POINT_SIZE_NOT_SIG), alpha = c(ALPHA_FDR, ALPHA_NOT_SIG), fill = c("black", "gray70")))) +
...
p_combined <- p_left + p_right + 
    plot_annotation(
        title = "Drop in Total Prescriptions due to Event, by Diagnosis Chapter",
        subtitle = sprintf("Number of diagnosis tested: %d", nrow(data_diag))
    )

# Save
ggsave(filename = paste0(OutDir, "Figure3A_V1_", DATE, ".png"), plot = p_combined, width = 19, height = 12, dpi = 300, device = "png")

# ============================================================================
# Version 2: show both FDR and nominally significant points with labels
# ============================================================================
...
# Left plot: Individual points
# select all FDR significant points and nominally significant points
top_extreme <- data_diag %>% filter(SIGNIFICANT_FDR | PVAL < 0.05)
...
# Save
ggsave(filename = paste0(OutDir, "Figure3A_V2_", DATE, ".png"), plot = p_combined, width = 19, height = 12, dpi = 300, device = "png")
