...
dataset$PVAL_ADJ <- p.adjust(dataset$PVAL_ABS_CHANGE, method = ADJ_METHOD)
dataset$SIGNIFICANT_CHANGE <- dataset$PVAL_ADJ < SIG_ALPHA

# Apply correction also to the pre and post event p-values
dataset$PVAL_PRE_ADJ <- p.adjust(dataset$PVAL_PRE, method = ADJ_METHOD)
dataset$PVAL_POST_ADJ <- p.adjust(dataset$PVAL_POST, method = ADJ_METHOD)
...
# -----------------------------------------------
# Per-medication stratified analysis
# -----------------------------------------------
result_list_1 <- list()
...
for (code in code_list) {
    tryCatch({
        ...
        p10 <- quantile(df_model$prescription_tier_value, probs = TIER_LOW_PROB, na.rm = TRUE)
        p90 <- quantile(df_model$prescription_tier_value, probs = TIER_HIGH_PROB, na.rm = TRUE)
        ...
        # ==========================================================
        # STEP 4: Stratified DiD analysis by tier
        # ==========================================================
        ...
        # ==========================================================
        # STEP 5: Combine tier results for this medication
        # ==========================================================
...
# -----------------------------------------------
# Combine across medications and save
# -----------------------------------------------
...
# Save final results
write.csv(results_wide, paste0(outdir, results_csv_file), row.names = FALSE)
...
results_wide <- read.csv(paste0(outdir, results_csv_file))
...
tier_labels <- results_wide %>%
    ...
    # rename low tier from "0-0" to "<1" for clarity
...
star_df <- results_wide %>%
    ...
    mutate(
        ...
        star = ifelse(!is.na(tier_pvalue) & tier_pvalue < SIG_ALPHA, "*", "")
    )
...
save_plot_png_pdf(forest_plot, outdir, basename_plot_file, width = PLOT_WIDTH, height = PLOT_HEIGHT, dpi = PLOT_DPI)