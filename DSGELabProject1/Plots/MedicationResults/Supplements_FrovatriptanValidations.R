...
outfile <- paste0(outdir, paste0("did_common_triptans_post2011_", DATE, ".csv"))
...
# -----------------------------------------------------------------------------
# 2. TRIPTAN NAME LOOKUP AND COLOUR PALETTES
# -----------------------------------------------------------------------------
...
palette_focal <- palette_all[focal_triptans]  


# -----------------------------------------------------------------------------
# 3. PLOT A — Triptan prescription landscape
# -----------------------------------------------------------------------------

focal_code   <- "N02CC07"
atc_prefix   <- substr(focal_code, 1, 5)   # "N02CC"
...
for (atc_group_code in atc_group_codes) {
    ...
    # TRIPTAN_NAME maps the ATC code to its generic drug name for axis labels.
    landscape_results[[atc_group_code]] <- df_model %>%
        group_by(YEAR) %>%
        summarise(
            CODE        = atc_group_code,
            TRIPTAN_NAME = all_triptan_names[atc_group_code],
...
# Build Plot A — colour by triptan name, not raw ATC code
plot_A <- ggplot(landscape_all, aes(x = YEAR, y = AVG_N, color = TRIPTAN_NAME, group = TRIPTAN_NAME)) +
    geom_line() +
    geom_point() +
...
ggsave(
    paste0(outdir, "Supplement_FrovatriptanPrescriptionLandscape_", DATE,".png"),
    plot_A, width = 10, height = 6, dpi = 300
)

# -----------------------------------------------------------------------------
# 4. PLOT B — Zoom-in DiD results for the three most common triptans
# -----------------------------------------------------------------------------

# Reload raw events and filter to the triptan class
events_raw2 <- as.data.table(read_parquet(events_file))
events_triptan <- events_raw2[SOURCE == "Purch" & startsWith(as.character(CODE), "N02CC")]

# The lookup and palette subset are already defined in Section 3 above.

did_results_list <- list()

for (outcome_code in names(focal_triptans)) {

    # --- Load outcomes for this triptan ---------------------------------------
    ...
    # prescription of exactly this triptan code
    events_focal <- events_triptan[, .(PATIENT_ID, CODE, DATE)]
    ...
    cat(sprintf("%-14s | Original range: %d-%d | Buffered range: %d-%d\n",
                focal_triptans[outcome_code], original_min_year, original_max_year,
                buffered_min_year, buffered_max_year))
    ...
    # Restrict to events that occurred after 2010 + 3 years (washout period before event) = 2013
    df_model <- df_model[is.na(EVENT_YEAR) | EVENT_YEAR > 2013]
...
data_plot[, outcome_label := focal_triptans[outcome_code]]

# Build Plot B — same colour hues as Plot A for the focal triptans
...
ggsave(
    paste0(outdir, "Supplements_DiD_MostCommonTriptans_", DATE, ".png"),
...
grob_A_no_legend <- arrangeGrob(plot_A_no_legend,
                                top = grid::textGrob("A. Triptan prescription landscape",
                                                     x = 0, hjust = 0,
                                                     gp = grid::gpar(fontface = "bold", fontsize = 13)))

grob_B_no_legend <- arrangeGrob(plot_B_no_legend,
                                top = grid::textGrob("B. Comparison of frovatriptan vs. sumatriptan (after 2013)",
                                                     x = 0, hjust = 0,
                                                     gp = grid::gpar(fontface = "bold", fontsize = 13)))
...
ggsave(
    paste0(outdir, "Supplements_FrovatriptanValidation_After2013_" ,DATE, ".png"),
