...
outfile = paste0(outdir, "did_case_cohort_estimates_", DATE, ".csv")
...
for (code in code_list) {
    ...
    if (!file.exists(outfile)) {
        ...
        write_csv(results, outfile, append = TRUE)
    }

    events_per_year <- df_model[df_model$EVENT == 1, .(N = uniqueN(DOCTOR_ID)), by = EVENT_YEAR][order(EVENT_YEAR)]
    events_year_str_1 <- paste0(events_per_year[EVENT_YEAR < 2005, ]$EVENT_YEAR, ":", events_per_year[EVENT_YEAR < 2005, ]$N, collapse = ", ")
    events_year_str_2 <- paste0(events_per_year[EVENT_YEAR >= 2005 & EVENT_YEAR < 2010, ]$EVENT_YEAR, ":", events_per_year[EVENT_YEAR >= 2005 & EVENT_YEAR < 2010, ]$N, collapse = ", ")
    events_year_str_3 <- paste0(events_per_year[EVENT_YEAR >= 2010 & EVENT_YEAR < 2015, ]$EVENT_YEAR, ":", events_per_year[EVENT_YEAR >= 2010 & EVENT_YEAR < 2015, ]$N, collapse = ", ")
    events_year_str_4 <- paste0(events_per_year[EVENT_YEAR >= 2015, ]$EVENT_YEAR, ":", events_per_year[EVENT_YEAR >= 2015, ]$N, collapse = ", ")
    ...
    p[[code]] <- ggplot(results, aes(x = time, y = att)) +
...
ggsave(filename = paste0(outdir, "Supplements_DiD_CaseCohortEffects_", DATE, ".png"), plot = combined_plot, width = 30, height = 15)
