...
write.csv(combined_results,
          paste0(outdir, "Supplements_StratifiedAnalysis_Specialty_V3_", DATE, ".csv"),
          row.names = FALSE)


# -----------------------------------------------
# STRATIFIED DOT PLOT: baseline + expected value (left) and relative change (right)
# -----------------------------------------------

# reload data if running separately from the above code block
combined_results <- read_csv(paste0(outdir, "Supplements_StratifiedAnalysis_Specialty_V3_", DATE, ".csv"), show_col_types = FALSE)
...
# --- 1. Build complete specialty x code grid and join results ---
# Apply multiple-testing correction within the stratified results
combined_results <- combined_results %>%
    mutate(p_value_adj = p.adjust(p_value, method = "bonferroni"),
           significant = !is.na(p_value_adj) & p_value_adj < 0.05)

plot_data <- plot_grid_df %>%
    left_join(combined_results, by = c("code", "specialty")) %>%
    mutate(
        # Mask estimates where sample sizes are below threshold
        across(
            c(absolute_change, absolute_change_se, relative_change, n_cases, n_controls, baseline),
            ~ ifelse(is.na(n_cases) | is.na(n_controls) | n_cases < 30 | n_controls < 30, NA_real_, .)
        ),
        # Expected value after change = baseline + absolute_change
        expected_value    = baseline + absolute_change,
        # 95% CI on the expected value (anchored at baseline + change ± 1.96*SE)
        expected_ci_lo    = baseline + absolute_change - 1.96 * absolute_change_se,
        expected_ci_hi    = baseline + absolute_change + 1.96 * absolute_change_se,
        # Relative change CI: propagate SE through relative_change / baseline
        ...
        # Significance flag (carry through, default FALSE for masked rows)
...
# Drop specialties that lost all estimates after masking
...
pt_size    <- 2.2
cap_height <- dodge_step * 0.35

# --- 4. Shared colour palette ---
code_colours <- setNames(
    RColorBrewer::brewer.pal(max(3, n_codes), "Set1")[seq_len(n_codes)],
    code_order
)

# Alpha helper: significant rows get full opacity; non-significant get 0.15
sig_alpha <- function(significant) ifelse(significant, 1.0, 0.15)

# --- 5. Left panel: baseline (filled dot) + expected value (open dot) ---
# Both dots share the same y position; connected by a dotted line.
# 95% CI whiskers on the expected-value dot only.
# Non-significant estimates are rendered at 0.15 alpha.

left_data <- plot_data %>% filter(!is.na(baseline) & !is.na(absolute_change))

left_plot <- ggplot(left_data) +
    # Dotted connector: baseline → expected value
    geom_segment(
        aes(
            x     = baseline,        xend = expected_value,
            y     = y_dodge,         yend = y_dodge,
            colour = code_label,
            alpha  = sig_alpha(significant)
        ),
        linetype  = "dotted",
        linewidth = 0.5
    ) +
    # 95% CI whisker line on expected value
    geom_segment(
        aes(
            x     = expected_ci_lo,  xend = expected_ci_hi,
            y     = y_dodge,         yend = y_dodge,
            colour = code_label,
            alpha  = sig_alpha(significant)
        ),
    ...
    geom_segment(
        aes(
            x = expected_ci_lo, xend = expected_ci_lo,
            y = y_dodge - cap_height, yend = y_dodge + cap_height,
            colour = code_label,
            alpha  = sig_alpha(significant)
        ),
        linewidth = 0.35
    ...
    geom_segment(
        aes(
            x = expected_ci_hi, xend = expected_ci_hi,
            y = y_dodge - cap_height, yend = y_dodge + cap_height,
            colour = code_label,
            alpha  = sig_alpha(significant)
        ),
    ...
    # Baseline: filled dot
    geom_point(
        aes(x = baseline, y = y_dodge, colour = code_label,
            alpha = sig_alpha(significant)),
        shape = 16, size = pt_size
    ) +
    # Expected value: open dot
    geom_point(
        aes(x = expected_value, y = y_dodge, colour = code_label,
            alpha = sig_alpha(significant)),
        shape = 21, size = pt_size, fill = "white", stroke = 0.8
    ) +
    # Y-axis: specialty labels on the LEFT panel
    scale_y_continuous(
    ...
    scale_colour_manual(
        values = code_colours, name = "Medication",
        guide  = guide_legend(
    ...
    labs(
        x     = "Prescription rate",
        y     = NULL,
        title = "Baseline (●) and expected value after event (○)"
    ) +
...
# --- 6. Right panel: relative change with 95% CI ---
# Y-axis labels suppressed (shown on left panel).
# Non-significant estimates are rendered at 0.15 alpha.

right_data <- plot_data %>% filter(!is.na(relative_change))

right_plot <- ggplot(right_data) +
...
# Left panel (baseline + expected) | Right panel (relative change); legend at bottom
combined_plot <- left_plot + right_plot +
    plot_layout(widths = c(2, 1), guides = "collect") &
    theme(legend.position = "bottom")

ggsave(
    filename = paste0(outdir, "StratifiedDotPlot_Specialty_V3", DATE, ".png"),
    plot     = combined_plot,
    width    = 16,
    height   = max(6, length(all_specialties_global) * 0.55 + 3),
    units    = "in",
    dpi      = 300
)