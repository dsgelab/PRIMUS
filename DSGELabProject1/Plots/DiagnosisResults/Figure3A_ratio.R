...
# Derive SE from the 95% CI when both bounds are available
dataset$SE <-abs(dataset$PCT_UPPER - dataset$PCT_LOWER) / (2 * 1.96)
dataset$PVAL <- 2 * (1 - pnorm(abs(dataset$PCT_CHANGE / dataset$SE)))
...
chapter_summary <- dataset %>%
    ...
    summarise(
        weight_sum = sum(1 / (SE^2)),
        ATT_MEAN = sum(PCT_CHANGE / (SE^2)) / weight_sum,
...
p_left <- ggplot(dataset, aes(x = x_jittered, y = PCT_CHANGE, color = CHAPTER_NAME)) +
  ...
  labs(
    ...
    y = "Percentage Drop in Total Prescription\n(Within the event year) "
  ) +
  theme_minimal() +
  theme(
...
p_combined <- p_left + p_right + 
  plot_annotation(
    title = "Percentage Drop in Total Prescriptions  due to Event, by Disease Chapter",
    subtitle = sprintf("Number of diseases tested: %d", nrow(dataset))
  )

# Save
ggsave(paste0(OutDir, "Figure3A_ratio_", DATE, ".png"), plot = p_combined, width = 24, height = 14, dpi = 300, device = "png")