...
exec > >(tee "preprocess_and_train_xgboost_${icd10_code_no_dot}.log") 2>&1

# Create csv for patients having a diagnosis with the given icd10 code
...
python3 preprocess_tabular.py --outdir XGBoost --inputfile "$icd10_code_no_dot"DiagnosesWithPrescriptions --suffix "$icd10_code_no_dot"
...
python3 train_xgboost.py --outdir XGBoost/results --trainfile XGBoost/xgboost_train_"$icd10_code_no_dot".csv --testfile XGBoost/xgboost_test_"$icd10_code_no_dot".csv \
    --balanced 0 --dropna 0 --valsize 0.1 --dsize 1 --nproc 16 --nsearch 100 --shapdsize 0.5 --fitlc 1 --suffix "$icd10_code_no_dot"
