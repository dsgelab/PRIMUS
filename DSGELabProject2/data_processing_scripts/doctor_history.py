...
def add_mean_yearly_past_prescriptions_column(doctor_presc_dates, doctor_event_history, diagnosis):
...
def save_history(events_filename, events_dtypes, code_column, group_column, date_column, history_column_prefix, outfile_label, diagnosis):
    ...
    for chunk in pd.read_csv(events_file, dtype=events_dtypes, usecols=events_dtypes.keys(), chunksize=CHUNK_SIZE):
        ...
        if outfile_label == "Prescription":
            ...
            for doctor_id, group in chunk.groupby("DOCTOR_ID")[date_column]:
    ...
    if outfile_label == "Prescription":
        add_mean_yearly_past_prescriptions_column(doctor_presc_dates, doctor_event_history, diagnosis)
    ...
    print(f"{outfile_label} history saved to {outfile}.")
