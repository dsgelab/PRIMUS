...
def train():
    ...
    plt.title("Best Test Mean AUC during Hyperparameter Search")
    ...
    auc_val_default = average_precision_score(y_val, default_model.predict_proba(X_val)[:, 1])
    auc_val_tuned = average_precision_score(y_val, tuned_model.predict_proba(X_val)[:, 1])

    print(f"\nValidation AUC (tuned): {auc_val_tuned:.3f}")
    print(f"Validation AUC (default): {auc_val_default:.3f}")
    print(f"Improvement in AUC with tuned hyperparameters: {auc_val_tuned - auc_val_default:.4f}\n")
    print(f"Choosing {'tuned' if auc_val_tuned > auc_val_default else 'default'} model.")

    model = tuned_model if auc_val_tuned > auc_val_default else default_model
    tuned_params = search.best_params_ if auc_val_tuned > auc_val_default else {}
    ...
    auc_train = average_precision_score(y_train, y_pred_train)
    auc_val = auc_val_tuned
    ...
    # mean_auprc = round(np.mean(bootstrapped_auprcs), 3)

    # plt.hist(bootstrapped_auprcs, bins=50)
    # # Show random AUPRC as a vertical line
    # plt.axvline(positive_rate, color="k", linestyle="dashed", linewidth=2, label=f"random, AUPRC={positive_rate}")
    # plt.axvline(mean_auprc, color="r", linestyle="dashed", linewidth=2, label=f"XGBoost, AUPRC={mean_auprc}")
    # plt.xlabel("AUPRC")
    # plt.ylabel("Frequency")
    # plt.legend()
    # plt.title("Bootstrapped AUPRC Histogram")
    # savefig(f"{args.outdir}/xgb_auprc_histogram_{current_datetime}.png")
    # print("AUPRC histogram saved.")
